    ...
    <table class="table table-hover m-0 tickets-list table-actions-bar dt-responsive nowrap" cellspacing="0" width="100%" id="datatable">
        ...
        <tbody>
            <?php foreach ($data as $row) { ?>
                <?php if($row['slug'] != 'admin' && $row['slug'] != 'login'  && $row['username'] != 'admin') { ?>
                    <tr>
                        <td><?php echo $x; $x++; ?></td>
                        <td><strong><?php echo $row['username']; ?></strong></td>

                        <?php 
                            if($row['slug'] == 'proposal') $process = "Proposal";
                            if($row['slug'] == 'database') $process = "Database";
                            if($row['slug'] == 'user') $process = "User";
                            if($row['slug'] == null) $process = "Null";
                            ...
                            <td>
                                <div class="btn-group dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        ...
                                        <button type="button" class="dropdown-item" id="deleteLogUser" data-id="<?php echo $row['id']; ?>" data-toggle="tooltip" data-placement="top" class="btn btn-sm btn-danger" title="" data-original-title="Delete"><i class="mdi mdi-delete mr-2 text-muted font-18 vertical-middle"></i> Delete</button>
